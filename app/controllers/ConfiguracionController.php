...
class ConfiguracionController extends SecureController{
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		if(!empty($this->search)){
			...
			$db->where("(configuracion_id LIKE ? OR texto_encabezado LIKE ?)", array("%$text%","%$text%"));
		}
		if(!empty($this->orderby)){ // when order by request fields (from $_GET param)
			$db->orderBy($this->orderby,$this->ordertype);
		}
		else{
			$db->orderBy('configuracion.configuracion_id', ORDER_TYPE);
		}
		if( !empty($fieldname) ){
		...
		$this->view->page_title ="Configuración";
		$this->view->render('configuracion/list.php' , $data ,'main_layout.php');
	}
	/**
     * View Record Action 
     * @return View
	...
	function view( $rec_id = null , $value = null){
		...
		if( !empty($value) ){
			...
			$db->where('configuracion.configuracion_id' , $rec_id);
		}
		$record = $db->getOne($tablename, $fields );
		if(!empty($record)){
			$this->view->page_title ="Ver";
			$this->view->render('configuracion/view.php' , $record ,'main_layout.php');
		}
		else{
			$page_error = null;
			if($db->getLastError()){
	...
	function add(){
		if(is_post_request()){
			...
			$db = $this->GetModel();
			$tablename = $this->tablename = 'configuracion';
			$fields = $this->fields = array('texto_encabezado'); //insert fields
			$postdata = $this->transform_request_data($_POST);
			$this->rules_array = array(
				'texto_encabezado' => 'required',
			);
			$this->sanitize_array = array(
				'texto_encabezado' => 'sanitize_string',
			);
				...
				if(!empty($rec_id)){
					if(is_ajax()){
						...
						if(!empty($this->redirect)){ 
							...
							redirect_to_page("configuracion");
						}
					}
					return;
				}
		...
		$this->view->render('configuracion/add.php' ,null,'main_layout.php');
	}
	/**
     * Edit Record Action 
	...
	function edit($rec_id = null){
		...
		$this->rec_id = $rec_id;
		$tablename = $this->tablename = 'configuracion';
		$fields = $this->fields = array('configuracion_id','texto_encabezado'); //editable fields
		if(is_post_request()){
			Csrf :: cross_check();
			$postdata = $this->transform_request_data($_POST);
			$this->rules_array = array(
		...
		if(!empty($data)){
			$this->view->render('configuracion/edit.php' , $data, 'main_layout.php');
		}
		else{
			if($db->getLastError()){
	...
	function delete( $rec_ids = null ){
		...
		foreach($arr_id as $rec_id){
			$db->where('configuracion.configuracion_id' , $rec_id,"=",'OR');
		}
