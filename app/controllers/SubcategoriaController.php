<?php 
/**
 * Subcategoria Page Controller
 * @category  Controller
 */
class SubcategoriaController extends SecureController{
	/**
     * Load Record Action 
     * $arg1 Field Name
     * $arg2 Field Value 
	...
	function index($fieldname = null , $fieldvalue = null){
		$db = $this->GetModel();
		$tablename = $this->tablename = 'subcategoria';
		$fields = array('subcategoria.subcategoria_id', 
			'subcategoria.nombre');
		$limit = $this->get_page_limit(MAX_RECORD_COUNT); // return pagination from BaseModel Class e.g array(5,20)
		$getdata = $this->getdata; //array of sanitized values passed via $_GET;
		if(!empty($this->search)){
			$text = trim($this->search);
			$db->where("(subcategoria.subcategoria_id LIKE ? OR subcategoria.nombre LIKE ?)", array("%$text%","%$text%"));
		}
		$db->orderBy('subcategoria.subcategoria_id', ORDER_TYPE);
		if( !empty($fieldname) ){
		...
		$this->view->page_title ="Subcategoria";
		$this->view->render('subcategoria/list.php' , $data ,'main_layout.php');
	}
	/**
     * View Record Action 
     * @return View
	...
	function view( $rec_id = null , $value = null){
		...
		if( !empty($value) ){
			...
			$db->where('subcategoria.subcategoria_id' , $rec_id);
		}
		$record = $db->getOne($tablename, $fields );
		if(!empty($record)){
			$this->view->page_title ="Ver";
			$this->view->render('subcategoria/view.php' , $record ,'main_layout.php');
		}
		else{
			$page_error = null;
			if($db->getLastError()){
	...
	function add(){
		if(is_post_request()){
			...
			$db = $this->GetModel();
			$tablename = $this->tablename = 'subcategoria';
			$fields = $this->fields = array('nombre','max_num','real_num'); //insert fields
		...
		$this->view->render('subcategoria/add.php' ,null,'main_layout.php');
	}
	/**
     * Edit Record Action 
	...
	function edit($rec_id = null){
		...
		$this->rec_id = $rec_id;
		$tablename = $this->tablename = 'subcategoria';
		$fields = $this->fields = array('subcategoria_id','nombre'); //editable fields
		if(is_post_request()){
			Csrf :: cross_check();
			$postdata = $this->transform_request_data($_POST);
			$this->rules_array = array(
		...
		if(!empty($data)){
			$this->view->render('subcategoria/edit.php' , $data, 'main_layout.php');
		}
		else{
			if($db->getLastError()){
	...
	function delete( $rec_ids = null ){
		...
		foreach($arr_id as $rec_id){
			$db->where('subcategoria.subcategoria_id' , $rec_id,"=",'OR');
		}
