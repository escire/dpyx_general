...
class FormularioController extends BaseController{
	...
	function index($fieldname = null , $fieldvalue = null){
		$db = $this->GetModel();
		$tablename = $this->tablename = 'formulario';
		$fields = array('formulario_id', 
			'descripcion', 
			'max_num', 
			'real_num', 
			'orden');
		...
		if(!empty($this->search)){
			...
			$db->where("(formulario_id LIKE ? OR descripcion LIKE ? OR max_num LIKE ? OR real_num LIKE ? OR orden LIKE ?)", array("%$text%","%$text%","%$text%","%$text%","%$text%"));
		}
		if(!empty($this->orderby)){ // when order by request fields (from $_GET param)
			$db->orderBy($this->orderby,$this->ordertype);
		}
		else{
			$db->orderBy('formulario.formulario_id', ORDER_TYPE);
		}
		if( !empty($fieldname) ){
		...
		$this->view->page_title ="Formulario";
		$this->view->render('formulario/list.php' , $data ,'main_layout.php');
	}
	/**
     * View Record Action 
     * @return View
	...
	function view( $rec_id = null , $value = null){
		...
		if( !empty($value) ){
			...
			$db->where('formulario.formulario_id' , $rec_id);
		}
		$record = $db->getOne($tablename, $fields );
		if(!empty($record)){
			$this->view->page_title ="Ver";
			$this->view->render('formulario/view.php' , $record ,'main_layout.php');
		}
		else{
			$page_error = null;
			if($db->getLastError()){
	...
	function add(){
		if(is_post_request()){
			...
			$db = $this->GetModel();
			$tablename = $this->tablename = 'formulario';
			$fields = $this->fields = array('descripcion','max_num','real_num','orden'); //insert fields
		...
		$this->view->render('formulario/add.php' ,null,'main_layout.php');
	}
	/**
     * Edit Record Action 
	...
	function edit($rec_id = null){
		...
		$this->rec_id = $rec_id;
		$tablename = $this->tablename = 'formulario';
		$fields = $this->fields = array('formulario_id','descripcion','max_num','real_num','orden'); //editable fields
		if(is_post_request()){
			Csrf :: cross_check();
			$postdata = $this->transform_request_data($_POST);
			$this->rules_array = array(
		...
		if(!empty($data)){
			$this->view->render('formulario/edit.php' , $data, 'main_layout.php');
		}
		else{
			if($db->getLastError()){
	...
	function delete( $rec_ids = null ){
		...
		foreach($arr_id as $rec_id){
			$db->where('formulario.formulario_id' , $rec_id,"=",'OR');
		}
