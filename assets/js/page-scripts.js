...
$(document).on('change', '[data-load-select-options]', function(e){
	...
	if(elementType == 'input'){
		var arrSelectedVals = [];
		$(this).closest('.form-group').find("input:checked").each(function(){
			arrSelectedVals.push($(this).val());
		});
		selectedVal = arrSelectedVals.toString();
	}
	else{
		selectedVal = $(this).val().toString();
	}
	var targetFields =  $(this).data('load-select-options');
	var arrFields = targetFields.split(",");
	arrFields.forEach(function(field){
		var targetSelectElem = $("#ctrl-" + field);
		var path = targetSelectElem.data('load-path') + '/' + encodeURIComponent(selectedVal);
		$(targetSelectElem).html('<option value="">Loading...</option>');
		var placeholder = $(targetSelectElem).attr('placeholder') || 'Select a value...';
		$.ajax({
			type: 'GET',
			url: path,
			dataType: 'json',
			success: function(data){
				if($(targetSelectElem).hasClass('selectize')){
					$(targetSelectElem).each(function() {
						if (this.selectize) {
...
$(document).on('change', '[data-load-check-options]', function(e){
	...
	var targetFields =  $(this).data('load-check-options');
	var arrFields = targetFields.split(",");
	arrFields.forEach(function(field){
		var targetOptionsHolder = $("#" + field + "-options-holder");
